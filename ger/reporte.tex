...
en una barra metalica. Se comparan una PINN clasica (M1), una PINN clasica
optimizada de forma independiente (M1B), una PINN con balance adaptativo de
terminos de perdida basado en estadisticas de gradientes (M2) y la solucion
analitica del problema. No se implementa la arquitectura nueva del paper.
Tambien se incluye busqueda bayesiana de hiperparametros y una comparacion de
optimizadores.
\end{abstract}
...
El modelo M1B usa exactamente la misma perdida que M1, pero sus hiperparametros
se eligen con una busqueda bayesiana propia. M1B no es una mejora propuesta por
el paper; es una ablacion agregada para separar el efecto del tuning del efecto
del balance adaptativo.
...
\end{equation}
En la corrida final se usa $\beta=0.9609403$. Las estadisticas se computan
sobre los pesos de las capas lineales, como en el repositorio de los autores.
...
La corrida principal usa Adam con learning rate inicial $0.0020667$, sin
weight decay, semilla 0, CPU y 16 threads. El scheduler es exponencial:
\begin{equation}
  \eta_k=\eta_0\left(0.9^{1/1000}\right)^k.
\end{equation}
El entrenamiento principal dura 3000 iteraciones y usa arquitectura
$[2,80,80,80,1]$.

La busqueda bayesiana principal usa Optuna/TPE con 100 trials y 800 iteraciones
por trial sobre M2. El espacio explorado fue:
\begin{center}
\begin{tabular}{ll}
\toprule
Parametro & Rango \\
...
Para M1B se repitio la busqueda con el mismo presupuesto, pero sin
$\beta$ adaptativo, porque M1B mantiene $\lambda_{\mathrm{ic}}=
\lambda_{\mathrm{bc}}=1$.
...
M1B & 0.182104 & 343.35 & 1.00 & 1.00 \\
M2 & 0.028535 & 211.49 & 154084.83 & 251158.37 \\
\bottomrule
\end{tabular}
\caption{Comparacion principal y ablacion M1B con 3000 iteraciones. M1 y M2
usan la arquitectura $[2,80,80,80,1]$; M1B usa la arquitectura optimizada
$[2,48,48,48,48,48,1]$.}
\end{table}

La mejora de M1 a M2 fue $9.26\times$ en error relativo $L^2$. La optimizacion
separada de M1 produjo M1B y mejoro a M1 por $1.45\times$, pero M1B todavia
quedo $6.38\times$ por encima del error de M2.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\linewidth]{outputs/comparacion_modelos_m1b.png}
\caption{Comparacion extra con M1B, la M1 optimizada sin pesos adaptativos.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{outputs/M1B_solucion.png}
\caption{Solucion exacta, prediccion M1B y error absoluto.}
\end{figure}
...
\caption{Figura resumen tipo paper: solucion exacta, M1, M2 y errores.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{outputs/M1B_gradientes.png}
\caption{Histogramas de gradientes por capa para M1B.}
\end{figure}
...
El mejor trial corto de la busqueda bayesiana fue el trial 91
($\mathrm{width}=80$, $\mathrm{depth}=4$, $\eta=0.001414$,
batch 384 y $\beta=0.96918$), con error corto 0.03197. Sin embargo, la
validacion larga de los 6 mejores trials mostro que el mejor regimen sostenido
fue el trial 82: $\mathrm{width}=80$, $\mathrm{depth}=3$,
$\eta=0.0020667$, batch 256 y $\beta=0.96094$. Este regimen obtuvo error
validado 0.02842 para M2.

Para M1B se hizo otra busqueda Optuna/TPE sin $\beta$ adaptativo. El mejor trial
corto fue el 91 ($\mathrm{width}=48$, $\mathrm{depth}=5$,
$\eta=0.0010009$, batch 128), con error corto 0.26293. La validacion larga de
los 6 mejores trials eligio el trial 83: $\mathrm{width}=48$,
$\mathrm{depth}=5$, $\eta=0.0020996$ y batch 128. Ese regimen obtuvo error
validado 0.18460 y error final 0.18210 en grilla $180\times180$.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{outputs/paper_balance_gradientes.png}
\caption{Magnitud media de gradientes por capa para M1 y M2.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{outputs/paper_lambda_evolucion.png}
\caption{Evolucion de pesos adaptativos en M2, analoga al grafico de lambdas del paper.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{outputs/sensibilidad_regimenes.png}
\caption{Sensibilidad entre error corto de Optuna y error validado a 3000 iteraciones.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{outputs/bayes_m1_historia.png}
\caption{Historia de trials y mejor error acumulado en la busqueda bayesiana de M1B.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{outputs/bayes_m1_parametros.png}
\caption{Error de M1B contra parametros explorados por Optuna/TPE.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{outputs/bayes_m1_heatmap_arquitectura.png}
\caption{Mejor error M1B observado por ancho y profundidad.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{outputs/validacion_regimenes.png}
\caption{Validacion larga de los mejores trials de Optuna y del regimen conservador.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.82\linewidth]{outputs/validacion_m1b.png}
\caption{Validacion larga de los mejores trials de Optuna para M1B.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{outputs/comparacion_semillas.png}
\caption{Robustez con tres semillas. M2: $0.0551\pm0.0232$; M1:
$0.2401\pm0.0225$.}
\end{figure}
...
\midrule
Adam, lr=$5\cdot10^{-4}$ & 0.167167 & 68.63 \\
AdamW, lr=$5\cdot10^{-4}$ & 0.167167 & 71.72 \\
Adam lr=$5\cdot10^{-4}$ + LBFGS(10) & 0.167244 & 69.13 \\
Adam, lr=$10^{-3}$ & 0.176340 & 69.26 \\
SGD, lr=$10^{-4}$, momentum=0.9 & 0.220924 & 70.58 \\
\bottomrule
\end{tabular}
...
\caption{Comparacion grafica de optimizadores con learning rates ajustados por familia.}
\end{figure}

\section{Graficos analogos al notebook de examen}

Se reviso el notebook de examen \texttt{jurado\_german\_parcial.ipynb},
ubicado en el directorio local \texttt{ejemplos\_figuras\_examen}.
Alli se grafican $f(x_k)$, $\|\nabla f(x_k)\|$ y trayectorias sobre contornos
para una funcion objetivo de dos variables. En una PINN no hay una trayectoria
2D natural en el espacio de pesos, porque la red tiene miles de parametros. Por
eso se trasladaron esos graficos al espacio interpretable de hiperparametros:
cada trial de Optuna es un punto de la trayectoria. Para M2 el plano usado es
$\log_{10}(\eta)$ contra $\beta$ adaptativo; para M1B, que no tiene
$\beta$, se usa $\log_{10}(\eta)$ contra ancho de red.

Las curvas de perdida de M1/M2 y la historia de Optuna cubren el analogo de
$f(x_k)$. Los gradientes se documentan con histogramas y magnitudes medias por
capa, que es el diagnostico usado por el paper. Como complemento, las siguientes
figuras muestran convergencia de Optuna y trayectorias tipo examen en el espacio
de busqueda.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{outputs/examen_bayes_convergencia.png}
\caption{Analogo de $f(x_k)$ para la busqueda bayesiana: error por trial y mejor acumulado.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{outputs/examen_bayes_trayectoria_lr_beta.png}
\caption{Trayectoria de Optuna sobre contornos interpolados de $\log_{10}(\mathrm{error})$
en el plano $\log_{10}(\eta)$-$\beta$.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{outputs/examen_bayes_m1_convergencia.png}
\caption{Analogo de $f(x_k)$ para la busqueda bayesiana de M1B.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{outputs/examen_bayes_m1_trayectoria_lr_width.png}
\caption{Trayectoria de Optuna para M1B en el plano $\log_{10}(\eta)$-ancho.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{outputs/examen_optimizadores_lr_error.png}
\caption{Sensibilidad de los optimizadores al learning rate y ranking con presupuesto fijo.}
\end{figure}
