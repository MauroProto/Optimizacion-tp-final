...
analitica de la ecuacion de calor. Como ablacion adicional, tambien se entrena
M1B: una M1 optimizada de forma independiente, sin pesos adaptativos.
\end{abstract}
...
Adicionalmente, se entreno M1B para separar el efecto del tuning de
hiperparametros del efecto del balance adaptativo. M1B mantiene
$\lambda_{ic}=\lambda_{bc}=1$ y usa una busqueda bayesiana propia.
...
un factor $9.26\times$. M1B mejora a M1 por tuning, pero queda $6.38\times$
por encima de M2.}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{outputs/comparacion_modelos_m1b.png}
\caption{Ablacion extra: comparacion de M1, M1B y M2.}
\end{figure}
...
bayesiana de hiperparametros. Para M2, el espacio explorado fue ancho de red,
profundidad, learning rate, batch y $\beta$ adaptativo. La busqueda corta eligio
como mejor trial al 91, pero la validacion larga selecciono el trial 82:
\[
...
La busqueda extra de M1B uso el mismo presupuesto, pero sin $\beta$. El mejor
regimen validado fue el trial 83:
\[
  \mathrm{width}=48,\quad
  \mathrm{depth}=5,\quad
  \eta=0.0020996,\quad
  \mathrm{batch}=128,
\]
con error final $0.1821$ en grilla $180\times180$.
...
\caption{Trayectoria de busqueda M1B en el plano $\log_{10}(\eta)$-ancho.}
\end{figure}
...
\begin{itemize}
  ...
  \item M1B optimiza la M1 clasica y baja el error a $0.1821$, por lo que el
  tuning ayuda pero no elimina la patologia.
